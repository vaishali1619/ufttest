...
Class DriverScript
	...
	Private Sub LoadSupportLibraries()
		...
		LoadFunctionLibrary "Support Libraries\Framework_Utilities\QCUtility.vbs"
		...
		If (Err.Number <> 0) Then
			...
			Else
				Select Case Environment.Value("OnError")
					'Stop option is not relevant when run from QC Test Lab
					Case "NextStep"
						gobjReport.UpdateTestLog "CRAFT Info", _
	...
	Private Sub WrapUp()
		...
		'If the user has not selected Temporary Run Results
		If Instr(Environment.Value("ResultDir"), "TempResults") = 0 Then
			UploadResultsToQc()
			UploadDataTablesToQc()
		End If
	...
	Private Sub UploadResultsToQc()
		...
		If (gobjReportSettings.ConsolidateScreenshotsInWordDoc) Then
			gobjQCUtility.AttachFolderToTestRun gobjReportSettings.ReportPath & "\Screenshots (Consolidated)"
		Else
	...
	Private Sub UploadDataTablesToQc()
		If Environment.Value("IncludeTestDataInReport") Then
			gobjQCUtility.AttachFileToTestRun gobjDataTable.DataTablePath & "\" & gobjDataTable.DataTableName & ".xls"
			gobjQCUtility.AttachFileToTestRun gobjDataTable.CommonDataTablePath & "\Common Testdata.xls"
		End If
	End Sub
	'###################################################################################################################
